document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyImagePreprocessing(ctx, w, h) {
        ...
        // === PASO 4: Auto-Inversión si está invertido ===
        // Los números de serie suelen ser texto NEGRO sobre fondo BLANCO/claro
        // Si hay más negro que blanco, probablemente está invertido
        if (blackCount > whiteCount) {
            console.log(`[OCR Preprocessing] Imagen invertida detectada (${blackCount} negro > ${whiteCount} blanco). Invirtiendo...`);
            for (let i = 0; i < d.length; i += 4) {
                d[i] = 255 - d[i];
                d[i + 1] = 255 - d[i + 1];
                d[i + 2] = 255 - d[i + 2];
