document.addEventListener('DOMContentLoaded', () => {
    ...
    function onScanSuccess(decodedText, decodedResult) {
        ...
        if (/^\d+$/.test(processedValue)) {
            ...
            // 2. Lógica Específica: "3 letras antes del symbolo y numeros finales"
            // El usuario mencionó "antes del $" (probablemente %) y "en vez de & poner -"
            // Basado en el código anterior y la petición, asumimos que el separador es '%' o un caracter especial.
            // Buscamos un patrón donde haya un separador (ej: %, $, -)

            // Intento 1: Buscar patrón exacto 3 letras + Separador + Numeros
            // Regex: 3 letras, seguido de cualquier cosa que no sea numero/letra (separador), seguido de numeros
            // O buscamos el símbolo '%' explícitamente como en la versión anterior

            if (decodedText.includes('%')) {
                // ...texto...ABC%12345...
                const parts = decodedText.split('%');
                // Tomamos la parte ANTES del primer % (o el último significativo)
                // El usuario dijo: "3 letras antes del ... y los numeros finales siguientes"

                // Busquemos el índice del %
                const separatorIndex = decodedText.indexOf('%');
                if (separatorIndex >= 3) {
                    // 3 chars antes
                    const prefix = decodedText.substring(separatorIndex - 3, separatorIndex);

                    // Numeros despues: extraer solo los dígitos que siguen inmediatamente o están al final
                    const suffixPart = decodedText.substring(separatorIndex + 1);
                    const numberMatch = suffixPart.match(/^(\d+)/); // Numeros inmediatamente despues del %
                    // O si estan al final del string: suffixPart.match(/(\d+)$/);

                    if (numberMatch) {
                        processedValue = `${prefix.toUpperCase()}-${numberMatch[1]}`;
                    } else {
                        // Fallback si no encuentra numeros pegados
                        processedValue = decodedText;
                    }
                }
            } else if (decodedText.includes('$')) {
                // Caso por si el usuario realmente tiene $ en el QR
                const parts = decodedText.split('$');
                const prefix = parts[0].slice(-3); // Ultimos 3 chars de la primera parte
                const suffix = parts[1].match(/^(\d+)/); // Numeros al inicio de la segunda
                if (prefix && suffix) {
                    processedValue = `${prefix.toUpperCase()}-${suffix[1]}`;
                }
            } else {
                // 3. Fallback a la lógica simple (ABC1234 -> ABC-1234) si no hay separadores especiales
                const simpleParts = processedValue.match(/^([A-Za-z]+)(\d+)$/);
                if (simpleParts) {
                    processedValue = `${simpleParts[1].toUpperCase()}-${simpleParts[2]}`;
                }
            }
        }
