document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyImagePreprocessing(ctx, w, h) {
        ...
        // Threshold para binarización (0-255). 
        // 100-128 suele ser buen punto de partida para texto negro sobre fondo claro.
        const threshold = 110;

        for (let i = 0; i < d.length; i += 4) {
            // 1. Grayscale (Luminosidad perceptual)
            const r = d[i];
            const g = d[i + 1];
            const b = d[i + 2];
            const gray = 0.2126 * r + 0.7152 * g + 0.0722 * b;

            // 2. Binarización (Black & White)
            // Si es más claro que el threshold, se vuelve blanco (255), si no negro (0)
            const val = gray > threshold ? 255 : 0;

            d[i] = val;     // R
            d[i + 1] = val; // G
            d[i + 2] = val; // B
            // d[i+3] es Alpha, no tocar
        }
