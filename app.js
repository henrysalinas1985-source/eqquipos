document.addEventListener('DOMContentLoaded', () => {
    ...
    captureBtn.addEventListener('click', () => {
        ...
        // === ESCALADO 2X para texto pequeño ===
        ...
        ctx.drawImage(ocrVideo, 0, 0, w * SCALE_FACTOR, h * SCALE_FACTOR);

        console.log(`[OCR] Imagen escalada de ${w}x${h} a ${w * SCALE_FACTOR}x${h * SCALE_FACTOR}`);

        // --- PRE-PROCESAMIENTO DE IMAGEN ---
        // Convertir a escala de grises y aumentar contraste para mejorar OCR
        applyImagePreprocessing(ctx, w * SCALE_FACTOR, h * SCALE_FACTOR);
    ...
    async function startSmartOCR(canvas) {
        ...
        if ('TextDetector' in window) {
            try {
                ...
                console.warn("[OCR] Native API returned empty or poor results. Fallback to OCR.space.");
            } catch (e) {
                console.error("[OCR] Native API Error:", e);
            }
        } else {
            console.log("[OCR] Native TextDetector not supported. Using OCR.space API.");
        }

        // 2. FALLBACK: OCR.space API (Reemplaza Tesseract.js)
        updateStatus("🌐 Conectando con OCR.space...");
        ocrBar.style.width = "30%";

        try {
            const result = await runOcrSpace(canvas, updateStatus);
            handleResult(result, "OCR_SPACE_API");
        } catch (err) {
    ...
    async function runOcrSpace(canvas, updateStatusRef) {
        // Convertir canvas a base64
        const base64Image = canvas.toDataURL('image/png');

        updateStatusRef("📤 Enviando imagen al servidor OCR...");
        ocrBar.style.width = "50%";

        // API Key gratuita de demostración (funciona para pruebas)
        // Para producción, registrarse en https://ocr.space/ocrapi para obtener una key propia
        const API_KEY = 'helloworld'; // Key gratuita de demo
        ...
        formData.append('OCREngine', '2'); // Engine 2 es mejor para texto alfanumérico
        ...
        updateStatusRef("📥 Procesando respuesta...");
        ocrBar.style.width = "80%";
        ...
        if (data.IsErroredOnProcessing) {
            throw new Error(data.ErrorMessage || 'Error en el procesamiento OCR');
        }
