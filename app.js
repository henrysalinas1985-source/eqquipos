document.addEventListener('DOMContentLoaded', () => {
    ...
    captureBtn.addEventListener('click', () => {
        ...
        if(statusText) statusText.textContent = "⏳ Inicializando motor OCR...";

        // OPTIMIZACIÓN: Whitelist para números y letras mayúsculas
        // Nota: Tesseract descarga archivos de lenguaje la primera vez. Requiere internet si no está en cache.
        Tesseract.recognize(
            ...
            'eng',
            {
                logger: m => { 
                    ...
                    if (m.status === 'recognizing text') {
                         ...
                         if(statusText) statusText.textContent = `⏳ Analizando texto... ${Math.floor(m.progress * 100)}%`;
                    } else if (m.status === 'loading tesseract core') {
                         if(statusText) statusText.textContent = "⏳ Cargando motor...";
                    } else if (m.status === 'loading language traineddata') {
                         if(statusText) statusText.textContent = "⏳ Descargando datos de idioma (requiere internet la 1ra vez)...";
                    }
                },
                // Relajamos un poco el whitelist por si detecta espacios o caracteres similares
            ...
            alert(`Error CRÍTICO de OCR:\n${err.message || err}\n\nSi es la primera vez, verifica tu conexión a internet para descargar el modelo de lenguaje.`);
            closeOcrModal();
        });
    });
