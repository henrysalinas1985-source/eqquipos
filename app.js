document.addEventListener('DOMContentLoaded', () => {
    ...
    function onScanSuccess(decodedText, decodedResult) {
        ...
        qrInput.value = decodedText; // Llenar el input
        stopScanning(); // Detener escaneo automáticamente

        // Feedback visual
        scanResult.classList.remove('hidden');

        // Detectar si es un Link (URL)
        const isUrl = decodedText.toLowerCase().startsWith('http://') || decodedText.toLowerCase().startsWith('https://');

        if (isUrl) {
            scanResult.innerHTML = `
                <div style="display:flex; flex-direction:column; align-items:center; gap:10px;">
                    <span>¡Enlace detectado!</span>
                    <a href="${decodedText}" target="_blank" class="action-btn" style="text-decoration:none; display:inline-block;">
                        🔗 Abrir Enlace
                    </a>
                    <span style="font-size:0.8em; color:#94a3b8;">${decodedText}</span>
                </div>
            `;
            // Opcional: Abrir automáticamente (puede ser bloqueado por pop-up blocker)
            // window.open(decodedText, '_blank');
        } else {
            scanResult.textContent = `¡Código detectado! ${decodedText}`;
        }

        // Ocultar mensaje después de unos segundos solo si NO es un link (para dar tiempo a clickear)
        if (!isUrl) {
            setTimeout(() => {
